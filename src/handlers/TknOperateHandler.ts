...
import { TknSystemHandler, TknDataTableHandler, KnUtility, KnValidateInfo, KnContextInfo, KnDataSet, KnLabelConfig, KnFormatInfo, KnDataTable, KnDataMapEntitySetting, KnDataEntity } from '@willsofts/will-core';
...
import { OPERATE_HANDLERS, QUERY_MODES } from "../models/EnsureAlias";
import path from 'path';

export class TknOperateHandler extends TknSystemHandler {
    ...
    public async buildHtml(viewfile: string, data: any, context: KnContextInfo, workdir?: string) : Promise<string> {
        ...
        try {
            ...
            return page.renderFile(viewfile, {data: data, page: page, label: label, meta: { language: language }});
        } finally {
            if(span) span.finish();
        }
    }
