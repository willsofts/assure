...
import { VerifyError, KnUtility } from "@willsofts/will-core";
...
export class KnPageRender extends KnPageFacility {
    ...
    public async renderHtml(viewfile: string, data: any) : Promise<string> {
        return this.renderFile(viewfile, {data: data, page: this, meta: { language: KnUtility.getDefaultLanguage(this.context) }});
    }

    public async renderFile(filename: string, data?: any, options?: any) : Promise<string> {
        if(filename.lastIndexOf(".ejs")<0) filename += ".ejs";
        let workdir = Utilities.getWorkingDir(path.dirname(__dirname)); 
