...
export class Sfte016Handler extends TknOperateHandler {
    ...
    public async doResetFactor(context: KnContextInfo, model: KnModel) : Promise<any> {
        let handler = new Sfte007Handler();
        return handler.doResetFactor(context, model);
    }
    ...
    public override async getDataSearch(context: KnContextInfo, model: KnModel) : Promise<KnDataTable> {
        ...
        return this.createDataTable(KnOperation.COLLECT, this.createRecordSet(rs), {}, "sfte016/sfte016_data");
    }
    ...
    public override async getDataRetrieval(context: KnContextInfo, model: KnModel) : Promise<KnDataTable> {
        ...
        try {
            ...
            if(rs.rows.length>0) {
                ...
                return this.createDataTable(KnOperation.RETRIEVAL, row, dt.entity, "sfte016/sfte016_dialog");
            }
            return this.recordNotFound();
        } catch(ex: any) {
            this.logger.error(this.constructor.name,ex);
    ...
    public override async getDataAdd(context: KnContextInfo, model: KnModel) : Promise<KnDataTable> {
        let ds = this.emptyDataSet();
        ds.gender = "M";
        return this.createDataTable(KnOperation.ADD, ds, {}, "sfte016/sfte016_dialog");
    }
    ...
    public async createUserTableToConfirm(context: KnContextInfo, model: KnModel, db: KnDBConnector, found: boolean, eng: boolean = true, status: string = "A") : Promise<KnRecordSet> {
    ...
    public async createUserTableToActivate(context: KnContextInfo, model: KnModel, db: KnDBConnector, found: boolean, eng: boolean = true, status: string = "P") : Promise<KnRecordSet> {
        let rs = await this.insertUserTable(context, model, db, found);
        if(rs && rs.rows.length>0) {
            let record = rs.rows[0];
            ...
            let info = handler.createActivateInfo({
                ...
                activateparameter: record.userpassword,
                activatemessage: record.activateurl,
                activateremark: record.email,
                activatetimes: 1,
            });
            info.activatelink = record.activateurl+"/account/"+info.activatekey;
            record.activatelink = info.activatelink;
            let [msg,tmp] = await this.composeMailActivate(db, record, eng);
            info.activatecontents = msg;
    ...
    public async composeMailActivate(db: KnDBConnector, record: any, eng: boolean = true, template: string = "USER_ACTIVATE", templatetype: string = "MAIL_ACTIVATE") : Promise<[string,KnTemplateInfo|undefined]> {
        let msg = undefined;
        let noti = new KnNotifyConfig();
        let tmp = await noti.getConfigTemplate(db, template, templatetype);
        if(tmp) {
        ...
        if(!msg || msg.trim().length==0) {
            msg = "Dear, ${userfullname}.<br/>";
            msg += "New account was created for access system.<br/>";
            msg += "To confirm, please kindly click link below to activate.<br/><br/>";
            msg += "<a href=\"${activatelink}\">Confirm click here</a><br/>"
            msg += "<br/>Regards,<br/>";
            msg += "Administrator<br/>";
        }
        return [Utilities.translateVariables(msg, record),tmp];	
    ...
    public async insertUserTable(context: KnContextInfo, model: KnModel, db: KnDBConnector, found: boolean, status: string = "A") : Promise<KnRecordSet> {
        ...
        try {
            if(!found) {
                ...
                knsql.append("usertname=?usertname, usertsurname=?usertsurname, ");
            ...
            knsql.append("values(?userid,?username,?site,?startdate,?status,?userpassword,?passwordexpiredate,'1',?editdate,?edittime,?edituser) ");
            knsql.set("userid",context.params.userid);
            knsql.set("username",context.params.username);
            knsql.set("site",site);
